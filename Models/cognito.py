...
class CognitoIdentityProviderWrapper:
    ...
    def respond_to_mfa_challenge(self, username, session, mfa_code):
        ...
        try:
            kwargs = {
                ...
                'ChallengeResponses': {
                    'USERNAME': username, 'SOFTWARE_TOKEN_MFA_CODE': mfa_code}}
            if self.client_secret is not None:
            ...
            if err.response['Error']['Code'] == 'ExpiredCodeException':
                ...
                logger.error(
                    "Couldn't respond to mfa challenge for %s. Error: %s: %s", username,
                    err.response['Error']['Code'], err.response['Error']['Message'])
                raise
        else:
            return auth_result
    ...
    def reset_password(self, username):
        try:
            ...
            if err.response['Error']['Code'] == 'CodeDeliveryFailureException':
                logger.warning(
                    "Couldn't deliver the password reset verification code. Please check your email or SMS settings."
                )
            elif err.response['Error']['Code'] == 'UserNotFoundException':
                logger.warning(
                    "Couldn't find a user matching that username."
                )
            else:
                logger.error(
                    "Couldn't send password reset request for %s. Error: %s: %s", username,
                    err.response['Error']['Code'], err.response['Error']['Message'])
                raise
        else:
            return f"Password reset request sent to {response['CodeDeliveryDetails']['Destination']} by {response['CodeDeliveryDetails']['DeliveryMedium'].lower()}"

    def confirm_reset_password(self, username, password, confirmation_code):
        try:
            ...
            logger.error(
                "Couldn't confirm password reset request. Error: %s: %s", err.response['Error']['Code'], err.response['Error']['Message']
            )
            raise
        else:
            return response
